          ...
          <div className="flex space-x-2 whitespace-nowrap">
            {banCount > 0 && (
              <Link href={`/@${playerName}/bans`}>
                <Badge
                  variant="secondary"
                  className="px-1 text-secondary-foreground/50"
                >
                  <Icons.ban className="w-4 h-4 mr-1" /> {banCount}{" "}
                  {q(dictionary.words.bans, banCount).toUpperCase()}
                </Badge>
              </Link>
            )}
            {muteCount > 0 && (
              <Link href={`/@${playerName}/mutes`}>
                <Badge
                  ...
                  className="px-1 text-secondary-foreground/50"
                >
                  <Icons.mute className="w-4 h-[0.9rem] mr-1" /> {muteCount}{" "}
                  {q(dictionary.words.mutes, muteCount).toUpperCase()}
                </Badge>
              </Link>
            )}
            {warnCount > 0 && (
              <Link href={`/@${playerName}/warns`}>
                <Badge className="px-1">
                  <Icons.warn className="w-4 h-4 mr-1" /> {warnCount}{" "}
                  {q(dictionary.words.warns, warnCount).toUpperCase()}
                </Badge>
              </Link>
            )}
            {kickCount > 0 && (
              <Link href={`/@${playerName}/kicks`}>
                <Badge
                  ...
                  className="px-1 text-secondary-foreground/50"
                >
                  <Icons.kick className="w-4 h-4 mr-1" /> {kickCount}{" "}
                  {q(dictionary.words.kicks, kickCount).toUpperCase()}
                </Badge>
              </Link>
            )}
          </div>
