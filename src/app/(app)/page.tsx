  ...
  return {
    ...
    openGraph: {
      ...
      description: p(siteConfig.openGraph.pages.main.description, {
        ...
        total:
          (await db.litebans_bans.count()) +
          (await db.litebans_mutes.count()) +
          (await db.litebans_warnings.count()) +
          (await db.litebans_kicks.count()),
      }),
    },
  };
}
    ...
    >
      <div className="mx-auto grid gap-4 sm:grid-cols-2 sm:w-[496px] lg:grid-cols-4 lg:w-[1024px]">
        <PunishmentTypeCard
          ...
          count={banCount}
          href="/bans"
          punishmentIcon={Icons.ban({
            color: "red",
            className:
              "size-[8.5rem] opacity-20 absolute top-[-35px] right-[-25px] blur-sm transition duration-700 group-hover:opacity-40 group-hover:scale-[1.15]",
          })}
        />
        <PunishmentTypeCard
        ...
        <PunishmentTypeCard
          ...
          count={warnCount}
          href="/warns"
          punishmentIcon={Icons.warn({
            className:
              "size-[8.5rem] opacity-20 absolute top-[-35px] right-[-25px] text-yellow-500 ml-2 blur-sm transition duration-700 group-hover:opacity-40 group-hover:scale-[1.15]",
          })}
        />
        <PunishmentTypeCard
          ...
          count={kickCount}
          href="/kicks"
          punishmentIcon={Icons.kick({
            className:
              "size-[8.5rem] opacity-20 absolute top-[-35px] right-[-25px] text-sky-600 ml-2 blur-sm transition duration-700 group-hover:opacity-40 group-hover:scale-110",
          })}
        />
      </div>
